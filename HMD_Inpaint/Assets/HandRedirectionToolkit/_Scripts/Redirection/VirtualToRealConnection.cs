...
namespace HR_Toolkit.Redirection
{
    ...
    public class VirtualToRealConnection : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            var visualizationLayer = LayerMask.NameToLayer("Visualization");
            if (visualizationLayer == -1)
            {
                if (Application.isPlaying)
                {
                    Debug.LogWarning("Layer 'Visualization' not found; falling back to current layer.");
                }
                visualizationLayer = gameObject.layer;
            }
            gameObject.layer = visualizationLayer;
